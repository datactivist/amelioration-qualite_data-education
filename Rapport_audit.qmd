...
effectifs_all <- bind_rows(
  effectifs_ecoles |> 
    filter(`Rentrée scolaire` >= "2019") |> 
    left_join(geoloc_eta |> select(`Numéro d'UAI`, `Latitude WGS84`, `Longitude WGS84`), 
              by = c("Numéro de l'école" = "Numéro d'UAI")) |> 
    select(`Rentrée scolaire`, `Numéro de l'école`, Secteur, `Nombre total d'élèves`, 
           `Latitude WGS84`, `Longitude WGS84`) |> 
    rename(`Nombre d'élèves` = `Nombre total d'élèves`, UAI = `Numéro de l'école`) |> 
    mutate(Niveau = "École", `Rentrée scolaire` = as.character(`Rentrée scolaire`)),
  effectifs_colleges |> 
    left_join(geoloc_eta |> select(`Numéro d'UAI`, `Latitude WGS84`, `Longitude WGS84`), 
              by = c("UAI" = "Numéro d'UAI")) |> 
    select(`Rentrée scolaire`, UAI, Secteur, nombre_eleves_total, 
           `Latitude WGS84`, `Longitude WGS84`) |> 
    rename(`Nombre d'élèves` = nombre_eleves_total) |> 
    mutate(Niveau = "Collège", 
           `Rentrée scolaire` = as.character(`Rentrée scolaire`),
           `Rentrée scolaire` = str_remove(`Rentrée scolaire`, "-01-01")),
  effectifs_lyceesGT |> 
    left_join(geoloc_eta |> select(`Numéro d'UAI`, `Latitude WGS84`, `Longitude WGS84`), 
              by = c("UAI" = "Numéro d'UAI")) |> 
    select(`Rentrée scolaire`, UAI, Secteur, `Nombre d'élèves`, 
           `Latitude WGS84`, `Longitude WGS84`) |> 
...
lv_all <- bind_rows(
  ...
  mutate(lv1_all = lv1_allemand + lv1_anglais + lv1_espagnol + lv1_autres_langues,
         lv2_all = lv2_allemand + lv2_anglais + lv2_espagnol + lv2_italien + lv2_autres_langues,
         lv1_allemand = lv1_allemand / lv1_all * 100,
         lv1_anglais = lv1_anglais / lv1_all * 100,
         lv1_espagnol = lv1_espagnol / lv1_all * 100,
         lv1_autres_langues = lv1_autres_langues / lv1_all * 100,
         lv2_allemand = lv2_allemand / lv2_all * 100,
         lv2_anglais = lv2_anglais / lv2_all * 100,
         lv2_espagnol = lv2_espagnol / lv2_all * 100,
         lv2_italien = lv2_italien / lv2_all * 100,
         lv2_autres_langues = lv2_autres_langues / lv2_all * 100) |> 
  select(-lv1_all, -lv2_all)
